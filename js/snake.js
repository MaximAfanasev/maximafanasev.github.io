...
function resetGame() {
    ...
    foodCount = 0; // Сбрасываем счетчик еды
}
...
function draw() {
    ...
    for (let i = 0; i < snake.length; i++) {
        ctx.beginPath();
        ctx.arc(snake[i].x + box / 2, snake[i].y + box / 2, box / 2, 0, Math.PI * 2); // Полный круг
    ...
    if (snakeX === food.x && snakeY === food.y) {
        foodCount++; // Увеличиваем счетчик еды
        food = generateFood(); // Создаем новую еду

        // Если змея собрала 20 или более точек, перезапускаем игру
        if (foodCount >= 50) {
