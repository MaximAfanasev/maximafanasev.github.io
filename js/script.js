...
Start.addEventListener('click', () => {
	...
	setTimeout(() => {
		calendarsContainer.classList.remove('hidden'); // Показываем контейнер с календарями
		...
		calendars.forEach((calendar) => {
			setTimeout(() => {
				calendar.style.opacity = 1; // Заменяем opacity на 1 для показа календаря
			}, delay);
			delay += 700; // Задержка 700 мс между появлениями календарей
		});
